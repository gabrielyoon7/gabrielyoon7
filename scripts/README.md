...
GitHub Pages 워크플로는 빌드·배포만 실행합니다. 기록 정리는 원하는 시점에 직접 실행합니다. 변경 내용을 먼저 커밋한 다음:

```sh
pnpm clear
```

현재 브랜치의 최신 파일을 커밋 하나로 남기고 `origin`의 같은 브랜치에 `--force-with-lease`로 push합니다. 커밋하지 않은 변경이나 원격에만 있는 이력이 있으면 중단합니다. `main`에 push하면 Pages 배포도 실행됩니다. 이 워크플로 변경을 `main`에 반영해야 자동 기록 정리가 중단됩니다.
